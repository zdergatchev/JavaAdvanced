    ...
    public static void main(String[] args) {
        ...
        for (String type : dragonTypes) {
            ...
            for (String name : nameSet) {
                ...
                sumHealt += currentDragon.get(1);
                sumArmor += currentDragon.get(2);
            }
            System.out.printf("%s::(%.2f/%.2f/%.2f)%n", type, sumDmg / names.size(),
                    sumHealt / names.size(), sumArmor / names.size());

            for (String name : nameSet) {
                ArrayList<Integer> currentDragon = names.get(name);
                System.out.printf("-%s -> damage: %d, health: %d, armor: %d%n", name, currentDragon.get(0), currentDragon.get(1), currentDragon.get(2));
            }
        }
    }
}
